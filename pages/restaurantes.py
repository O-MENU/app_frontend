...
def app():
    try:
        restaurantes = requests.get(url).json()
    except:
    ...
    op_filtro = ["ID", "Nome", "Localizacao", "CNPJ", "Menu"] # Opções de filtro
    db_op_filtro = ["_id", "nome", "Localizacao", "CNPJ", "Menu"] # Opções de filtro como estão na base de dados
    ...
    df_users = pd.DataFrame(restaurantes) # DataFrame de usuários
    ...
    if len(filtered_df_users) > 0:
        ...
        fields = ["ID", "Nome", "Localizacao", "CNPJ", "Menu", "Editar", "Apagar"] # Colunas + botao de editar + botao de apagar
        ...
        for i, id in enumerate(filtered_df_users["_id"]):
            ...
            col3.write(filtered_df_users["localizacao"][i])#    }   Colunas do DataFrame
